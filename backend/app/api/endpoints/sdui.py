...
from app.schemas.sdui_config import SDUIConfig, SDUIConfigCreate, SDUIConfigUpdate, SDUIConfigQuery
...
def get_structure(
    ...
    if config_type and theme:
        ...
        specific_config = crud.sdui_config.get_by_code(
            db=db, 
            code=f"structure-{config_type}-{theme}"
        )
        if specific_config:
            return specific_config.content
    elif config_type:
        # 尝试查找带有类型的配置
        specific_config = crud.sdui_config.get_by_code(
            db=db, 
            code=f"structure-{config_type}"
        )
        if specific_config:
            return specific_config.content
    ...
    if not structure_config:
        system_structure_config = crud.sdui_config.get_by_code(db=db, code="system-structure")
        if system_structure_config:
            return system_structure_config.content
    ...
    # 返回配置数据
    return structure_config.content
